...
export class BuybackApprovalHandler extends InteractionHandler {
	...
	public override async parse(interaction: ButtonInteraction) {
		if (!interaction.customId.startsWith('buyback-')) return this.none();
		this.container.logger.debug(`Parsing buyback approval interaction with custom ID ${interaction.customId}`);

		const [action, userId, totalCost] = interaction.customId.split(':');
		if (!['buyback-approve', 'buyback-reject'].includes(action)) return this.none();
	...
	public async run(interaction: ButtonInteraction, data: { action: string; userId: string; member: GuildMember; totalCost: string }) {
		this.container.logger.debug(`Attempting to handle buyback for ${data.userId}`);
		...
		this.container.logger.debug(`Handling buyback with action ${data.action}`);

		if (data.action === 'buyback-approve') {
