...
export class FinishEventHandler extends InteractionHandler {
	...
	public override parse(interaction: ButtonInteraction) {
		if (!interaction.customId.startsWith('event-finish')) return this.none();
	...
	public async run(interaction: ButtonInteraction, eventId: string) {
		...
		try {
			...
			if (event.userId !== interaction.user.id) {
				return interaction.reply({
					content: '❌ Only the creator of this event can finish it',
					flags: MessageFlags.Ephemeral
				});
			}
			...
			return interaction.reply({
				content: '✅ Successfully finished the event!',
				flags: MessageFlags.Ephemeral
			});
		} catch (error) {
